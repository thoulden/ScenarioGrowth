...
function createOutputChart(results, years) {
    ...
    var isWorld = typeof getRegionMode === 'function' ? getRegionMode() === 'global' : (!isUSMode() && !(typeof isChinaMode === 'function' && isChinaMode()));
    var datasets = [
        ds('Model Prediction', results.map(function(r) { return r.Y_predicted || r.Y; }), T.cognitive)
    ];
    if (isWorld) {
        datasets.push(ds('GATE Output', GATE_OUTPUT.slice(0, years.length), T.green, { dash: [10, 5] }));
    }
